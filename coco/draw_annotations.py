...
def draw_bezier_curve(ax, points, color, linewidth=1.5):
    """Draw a cubic Bezier curve given 4 (x, y) control points."""
    verts = [points[0], points[1], points[2], points[3]]
    codes = [Path.MOVETO, Path.CURVE4, Path.CURVE4, Path.CURVE4]
    path = Path(verts, codes)
    patch = patches.PathPatch(path, facecolor='none', edgecolor=color, linewidth=linewidth)
    ax.add_patch(patch)
...
def draw_annotations(image_path, annotation_json, image_id=None):
    ...
    for i, ann in enumerate(anns):
        ...
        # Draw the actual Bezier curves
        draw_bezier_curve(ax, upper, colors_upper)
        draw_bezier_curve(ax, lower, colors_lower)

        # Draw upper control points + dashed control polygon
        upper_x = [p[0] for p in upper]
        upper_y = [p[1] for p in upper]
        ax.plot(upper_x, upper_y, '--', color=colors_upper, linewidth=0.7, alpha=0.5)
        for k, (x, y) in enumerate(upper):
            ax.plot(x, y, 'o', color=colors_upper, markersize=6)

        # Draw lower control points + dashed control polygon
        lower_x = [p[0] for p in lower]
        lower_y = [p[1] for p in lower]
        ax.plot(lower_x, lower_y, '--', color=colors_lower, linewidth=0.7, alpha=0.5)
